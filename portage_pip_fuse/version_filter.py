...
class VersionFilterPythonCompat(VersionFilterBase):
    ...
    def _get_system_python_versions(self) -> Set[str]:
        ...
        # Fallback: check installed Python versions
        versions = set()
        for minor in range(8, 13):  # 3.8 through 3.12
            python_exe = f"python3.{minor}"
    ...
    def should_include_version(self, pypi_name: str, version: str, metadata: Dict) -> bool:
        """Check if version is compatible with system Python."""
        ...
        # Check if any system Python satisfies the requirement
        try:
            from packaging.specifiers import SpecifierSet
            spec = SpecifierSet(requires_python)
            
            for py_version in self.supported_pythons:
                if py_version in spec:
                    return True
            
            logger.debug(f"Version {version} of {pypi_name} requires Python {requires_python}, "
                        f"not compatible with system {self.supported_pythons}")
            return False
